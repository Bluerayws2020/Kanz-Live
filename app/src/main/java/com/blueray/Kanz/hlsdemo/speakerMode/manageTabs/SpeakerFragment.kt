...
class SpeakerFragment : Fragment() {
    ...
    private fun showMeetingInfo() {
        ...
        message.setTextColor(resources.getColor(R.color.grey))
        message1.text = "               " + (activity as MainActivitys?)!!.getMeeting()!!.meetingId
        message1.setOnTouchListener { v: View?, event: MotionEvent ->
            if (event.action == MotionEvent.ACTION_DOWN) {
                if (event.rawX >= message1.right - message1.compoundDrawables[2]
                    ...
                    copyTextToClipboard(
                        (activity as MainActivitys?)!!.getMeeting()!!.meetingId
                    )
                    return@setOnTouchListener true
                }
            }
